      ...
      return (
        ...
        //   <Navbar />
          <div className="flex flex-wrap justify-center  bg-slate-800 ">
            {/* className="max-w-md mx-4 my-6 rounded overflow-hidden shadow-lg" max-w-md */}
            
            {movies &&
             movies.map((movie) => (
            
               <div onClick={()=>{HandleChange(movie.id)}} className=" w-[150px] h-[200px] hover:w-[160px] hover:shadow-slate-700  mx-4 my-6 rounded overflow-hidden shadow-lg  bg-no-repeat bg-cover" style={{ backgroundImage: `url(https://image.tmdb.org/t/p/original/${movie.poster_path})` } }>
                
                  {/* <img src={`https://image.tmdb.org/t/p/original${movie.poster_path}`} alt={movie.title} className="w-full h-64 object-cover" /> */}
                  <div className="px-6 py-4 hidden hover:block ">
                    <h5 className="text-xl font-semibold mb-2">{movie.title}</h5>
